...
The initial plan was much bigger and more complex than what it turned out to be - I will talk about this in the section about [Potential improvements](#potential-improvements-and-additional-functionalities)
...
The app starts with an index page where the rules of the game are explained and some thanks are given.
...
After importing the necessary classes and modules and configuring the application and session, an API key and game_database.db are stored in variables.

A series of helper functions follows. Two of them are meant to ensure that a round stays consistent at every step by tracking properly the active players. One is meant to take care of picking a random actor.
...
game_database.db is an SQLite3 database tracking all the data.
...
* actors: keeps track of the guessed actors to avoid them getting picked twice by the app
...
## Potential improvements and additional functionalities
...
3. At the very beginning I had imagined the game completely customisable. For example, for added difficulty or vice-versa to make it easier in some cases, players could pick just a range of years in which the movies have been produced, or just a geographical area, or spoken in a certain language.
...
5. Sounds would possibly also add to the enjoyment of the game.


## Thoughts about the game, difficulties, solutions

I will keep this section short, but I took some notes in a diary as I was encountering difficulties and changing the direction of my work to adapt. This has been very useful in understanding how it's better to build up from the small to the big, instead of imagining already the final product in mind the whole time. I hope this can help me or others to be aware of this sort of trap.

Some of the notes.

> I managed to reduce the whole database to 1.67gB
Still quite big, but I hope it can still work.
But definitely more manageable than the 7 GB one I had at the start.
...
Here I was basically attempting to build my own specific database, instead of using an API!!! This was not only an incredibly silly approach, but it also brought me into unexpected places that wasted a lot of my time. For example, I started to test and study Oracle Cloud and many other things related to databases and database hosting.
...
Once I changed my approach, I have started to finally move on and build properly my app, thinking of details like "How the app should pick an actor?", "How the selection in the query should work? Could a user use both the mouse and the keyboard? Would it be useful to autoselect the search bar for keyboard-only users?". Also, by analysing David's code for the Finance pset, I started to explore better the choices related to security.
...
>   - Adding an original title to the movie when it's not the same as English (Nikita)
